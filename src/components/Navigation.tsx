        ...
        <div className="hidden md:flex items-center gap-2">
          {links.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="font-mono text-sm tracking-wider text-text-secondary hover:text-ice-400 transition-colors duration-300 px-4 py-2 rounded-md hover:bg-ice-400/5"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="bg-deep-navy/95 backdrop-blur-md border-t border-card-border/30 px-6 py-4 space-y-1">
          {links.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="block py-3 px-2 font-mono text-sm text-text-secondary hover:text-ice-400 transition-colors"
